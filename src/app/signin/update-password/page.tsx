...
export default function UpdatePasswordPage() {
  ...
  useEffect(() => {
    const checkRecovery = async () => {
      ...
      if (!session) {
        setMessage('No hi ha sessió activa. Torna a demanar el reset de contrasenya.');
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      setMessage(error.message || 'Error actualitzant contrasenya.');
    } else {
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-md p-8 bg-white/80 backdrop-blur-md shadow-lg rounded-2xl">
        <h1 className="text-3xl font-bold text-gray-900 mb-6 text-center">Restableix la teva contrasenya</h1>
        {message && <p className={`mb-4 text-center ${message.includes('correctament') ? 'text-green-600' : 'text-red-600'}`}>{message}</p>}

        <form onSubmit={handleSubmit}>
          <input type="password" placeholder="Nova contrasenya" value={password} onChange={(e) => setPassword(e.target.value)}
          ...
          <input type="password" placeholder="Repeteix la contrasenya" value={confirm} onChange={(e) => setConfirm(e.target.value)}
                 className="w-full p-2 mb-4 border border-gray-300 rounded placeholder-gray-500 text-black focus:outline-none focus:ring-2 focus:ring-rose-500" required />
