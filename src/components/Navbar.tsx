...
export default function Navbar() {
  ...
  const [isTop, setIsTop] = useState(true) // 👈 Track top of page
  ...
  useEffect(() => {
    const loadUser = async () => {
      ...
      try {
        const res = await fetch('/api/auth', { headers: { Authorization: `Bearer ${session.access_token}` }})
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (scrollY > lastScrollY.current && scrollY > 80) setShowNavbar(false)
      ...
      if (searchOpen && scrollY > lastScrollY.current) setSearchOpen(false)
    }
  ...
  const handleLogout = async () => { await supabase.auth.signOut(); resetCart(); window.location.href = '/' }

  return (
    <>
      <div
        className={`fixed top-0 left-0 w-full z-50 transition-transform duration-300`}
        style={{ transform: showNavbar ? 'translateY(0)' : 'translateY(-120%)' }}
      >
        <nav
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4 md:gap-6 absolute right-4">
              ...
              {user === null && <Link href="/"><User className="w-6 h-6 text-gray-700 hover:text-rose-600 cursor-pointer transition-all"/></Link>}
              {user && (
                <div className="relative" ref={menuRef}>
                  ...
                  {menuOpen && (
                    <div className="absolute right-0 mt-2 w-36 bg-white border border-gray-200 rounded-lg shadow text-sm z-50">
                      <Link href="/user" className="block px-4 py-2 hover:bg-rose-50 text-gray-700" onClick={() => setMenuOpen(false)}>Área Usuario</Link>
                      <button onClick={handleLogout} className="block w-full text-left px-4 py-2 hover:bg-rose-50 text-gray-700">Cerrar Sesión</button>
                    </div>
                  )}
                </div>
              )}
      ...
      {/* Search */}
      {searchOpen && (
        <div ref={searchContainerRef} className="fixed top-[72px] left-0 w-screen bg-[#f3e9dc] z-50 shadow-md border-t border-gray-200 px-4 md:px-8 py-3">
          <div className="relative w-full">
            ...
            <button onClick={() => setSearchOpen(false)} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-700">
              <X className="w-6 h-6"/>
            </button>
          </div>
